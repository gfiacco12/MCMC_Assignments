...
def fisher():
    mu,nu,sig, x = symbols('mu,nu,sig, x', real=True)
    f = symbols('f', cls=Function)
    pi = (1 / ( np.sqrt(2*np.pi)*sig))*(1 + (1/nu)*((x - mu)/sig)**2)**(-0.5*(nu+1))
    f = sp.log(pi)
    H = -hessian(f, [mu, nu, sig])
    #now evaluate the hessian at the expected values of mu, nu, sig
    #new_h = compile(str(H), 'test', 'eval')
    #fisher = -eval(new_h,{"mu": 1, "nu": 3, "sig": 1})
    print(H(1, 3, 1))
    return()
fisher()
